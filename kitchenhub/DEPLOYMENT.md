...
| Image | Description |
|-------|-------------|
| `derpmhichurp/kitchenhub-backend` | Express API |
| `derpmhichurp/kitchenhub-frontend` | React SPA (nginx) |
...
- **Version tags** (e.g., `kitchenhub/1.0.0`): Builds on any branch. Image is always tagged with the version (e.g. `1.0.0`).
...
**Creating a Release:**
```bash
git tag kitchenhub/1.0.0
git push origin kitchenhub/1.0.0
```

Example image names:
- `derpmhichurp/kitchenhub-backend:latest` (only when tag is on main and stable)
- `derpmhichurp/kitchenhub-backend:beta` (when tag is on a branch other than main)
- `derpmhichurp/kitchenhub-backend:1.0.0`
- `derpmhichurp/kitchenhub-backend:1.0.0-beta.1` (pre-release)
