...
Use the **Scenarios** page to create and edit named what-if plans (retirement ages, Social Security claim ages 62–70, spending, withdrawal strategy including custom bucket order, and Roth conversion plans). A default **Baseline** scenario is seeded from household settings. Compare scenarios on **Scenarios → Compare** for lifetime metrics and year-by-year tax deltas. **Projections** charts the selected scenario. For taxable brokerage accounts, set blended gain % and dividend yield when editing the account on **Accounts**. Not tax advice—federal ordinary + simplified LTCG only; no state tax, NIIT, or AMT.
