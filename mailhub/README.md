...
**Spam shows "DiscardedOpenRelay" and never reaches quarantine:** Amavisd treats the recipient as "nonlocal" and discards instead of quarantining. Set **`AMAVISD_LOCAL_DOMAINS`** to the domain of your local recipients (e.g. `plud.org` so that `user@muletrain.plud.org` is considered local). In the stack env add e.g. `AMAVISD_LOCAL_DOMAINS=plud.org`, then rebuild the amavisd image (so the updated config is in the image), pull/restart the container. You can also set **`MYDOMAIN`** to the same value (used by Postfix and by amavisd when `AMAVISD_LOCAL_DOMAINS` is unset).
