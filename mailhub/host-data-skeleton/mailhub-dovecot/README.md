...
- **users** — Copy from `users.example` to `users` and add your accounts. Format:  
  ...
  Use the **local part** as `user` (e.g. `user1` for `user1@localhost`).  
...
Uid/gid 5000:5000 match the image’s `vmail` user. Home must be under `/home/mailhub-dovecot` (e.g. `.../maildir/user1`).
